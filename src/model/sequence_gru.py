...
class GRUPairwiseRanking(nn.Module):
    def __init__(
        self, num_items, embedding_dim, device="cpu", max_input_sequence_length=5
    ):
        super(GRUPairwiseRanking, self).__init__()
        self.device = device
        ...
        # GRU layer to process the input sequence
        self.gru = nn.GRU(
        ...
        # Fully connected layer to map GRU output + target embedding to rating prediction
    ...
    def forward(self, input_seq, target_item):
        ...
        # Pass embedded sequence through GRU
        gru_output, hn = self.gru(
            embedded_seq
        )  # gru_output: [batch_size, seq_len, hidden_size]

        # Use the last hidden state as the sequence representation
        pooled_output = hn[-1]  # Shape: [batch_size, embedding_dim]
        ...
        # Concatenate the sequence representation with the target item embedding
        ...
        return output_ratings  # Shape: [batch_size, 1]
    ...
    def predict_train_batch(self, batch_input: dict):
        ...
        return pos_predictions.squeeze(1), neg_predictions.squeeze(2)
    ...
    def recommend(
        ...
        with torch.no_grad():
            ...
            for i in tqdm(
                ...
                # Expand item_sequences to match all items
...
class GRURatingPrediction(GRUPairwiseRanking):
    def predict_train_batch(self, batch_input: dict):
